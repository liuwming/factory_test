...
for mon in $(ifconfig | grep mon | cut -d' ' -f1)
do 
    airmon-ng stop $mon
done
...
EOF
    ...
    echo "dhcp-range=192.168.$((i+ip_sub_net_base)).10,192.168.$((i+ip_sub_net_base)).254,10m" >> /etc/dnsmasq.conf
...
for card in $wifi_cards
do 
    ifconfig $card 192.168.$((i+ip_sub_net_base)).1/24 up
    if [ $? -ne 0 ]
    then 
